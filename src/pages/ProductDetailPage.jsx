...
const ProductDetailPage = ({ product, setPage, setSelectedProduct }) => {
  ...
  const renderCategorySpecificDetails = () => {
    switch (product.category) {
      ...
      case 'Skins':
        return (
            <motion.div variants={itemVariants} className="mb-8 p-4 bg-gray-900/50 rounded-lg border border-gray-800">
                <h3 className="text-lg font-semibold mb-2 flex items-center"><Film className="mr-2 text-blue-400"/>Material</h3>
      ...
      case 'Accessories':
         return (
            <motion.div variants={itemVariants} className="mb-8 p-4 bg-gray-900/50 rounded-lg border border-gray-800">
                <h3 className="text-lg font-semibold mb-2 flex items-center"><Cpu className="mr-2 text-blue-400"/>Compatibility</h3>
  ...
  const allTechSpecs = {
    Watches: [
      { name: 'Display', value: '1.9" AMOLED Always-On', icon: <Maximize size={24} className="text-blue-400"/> },
      { name: 'Materials', value: 'Titanium Case, Sapphire Glass', icon: <ShieldCheck size={24} className="text-blue-400"/> },
      { name: 'Connectivity', value: 'Bluetooth 5.2, Dual-Band GPS', icon: <Zap size={24} className="text-blue-400"/> },
      { name: 'Battery Life', value: 'Up to 14 Days', icon: <BatteryCharging size={24} className="text-blue-400"/> },
      { name: 'Water Resistance', value: '5 ATM', icon: <Droplets size={24} className="text-blue-400"/> },
      { name: 'Processor', value: 'Anarc Fusion Chip', icon: <Cpu size={24} className="text-blue-400"/> },
    ],
    Skins: [
      { name: 'Material', value: 'Premium 3M Vinyl', icon: <Film size={24} className="text-blue-400"/> },
      { name: 'Finish', value: 'Matte with True Texture', icon: <Layers size={24} className="text-blue-400"/> },
      { name: 'Feature', value: 'Air-Release Adhesive', icon: <Wind size={24} className="text-blue-400"/> },
      { name: 'Durability', value: 'Scratch & Fade Resistant', icon: <ShieldCheck size={24} className="text-blue-400"/> },
      { name: 'Removal', value: 'Residue-Free', icon: <Trash2 size={24} className="text-blue-400"/> },
      { name: 'Precision', value: 'Laser Cut Accuracy', icon: <Ruler size={24} className="text-blue-400"/> },
    ],
    Accessories: [
      { name: 'Material', value: 'Liquid Silicone Rubber', icon: <Droplets size={24} className="text-blue-400"/> },
      { name: 'Clasp', value: 'Stainless Steel Pin-and-Tuck', icon: <Lock size={24} className="text-blue-400"/> },
      { name: 'Compatibility', value: '22mm Lug Width', icon: <Cpu size={24} className="text-blue-400"/> },
      { name: 'Feature', value: 'Sweat & Water Resistant', icon: <ShieldCheck size={24} className="text-blue-400"/> },
      { name: 'Design', value: 'Quick-Release Pins', icon: <Zap size={24} className="text-blue-400"/> },
      { name: 'Sizing', value: 'Fits 140-220mm wrists', icon: <Ruler size={24} className="text-blue-400"/> },
    ]
  };
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="visible">
              <motion.button onClick={() => setPage('Products')} className="flex items-center space-x-2 text-gray-400 mb-6 hover:text-blue-400 transition-colors" variants={itemVariants}>
                <ArrowLeft size={16}/>
              ...
              <motion.div variants={itemVariants} className="space-y-4">
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  className={`w-full ${cartAdded ? 'bg-green-600' : 'bg-blue-600 hover:bg-blue-700'} text-white font-bold px-8 py-4 rounded-xl text-lg flex items-center justify-center space-x-3 transition-all duration-300 shadow-lg shadow-blue-500/20`}
                >
          ...
          >
            ...
            <motion.div className="flex flex-col items-center" variants={itemVariants}>
              <Repeat2 className="text-blue-400 mb-3" size={32} />
            ...
            >
              {relatedProducts.map((related) => (
                ...
                >
                  ...
                  <p className="text-blue-400">₹{related.price.toLocaleString()}</p>
                </motion.div>
              ))}
            </motion.div>
          </div>
